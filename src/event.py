...
def load_properties():
    ...
    connect_params['zenoss_to_alerta'] = config.getboolean('system', 'zenoss_to_alerta')
...
def send_events_to_alerta(events):
    logger.info('total event to send: {}'.format(len(events)))
    for event in events:
        alerta.send_event_to_alerta(event, connect_params['dest_host'])
    logger.info('end')
...
if __name__ == "__main__":
    ...
    try:
        zenoss_to_alerta = connect_params['zenoss_to_alerta']
        if zenoss_to_alerta:
            zenoss_src = connection_setup(host=connect_params['src_host'])
            events_src_new = get_events(zenoss_src, [EventState.new])
            events_src_acks = get_events(zenoss_src, [EventState.acknowledged])
            events_src_closed = get_events(zenoss_src, [EventState.closed])

            events_src = []
            events_src.extend(events_src_new)
            events_src.extend(events_src_acks)
            events_src.extend(events_src_closed)

            send_events_to_alerta(events_src)
            sys.exit(0)
